...
`tests/qpic/` contains a checked `.qrab` translation for every one of qpic's 44 golden-test stems, and `tests/qpic-manual/` contains all 64 documented example stems. `tests/artifacts.rs` requires both exact lists and compiles them plus 11 focused qrab examples through both backends: 119 source fixtures and 238 PDFs per artifact run. Stored visual baselines remain Phase 4 work; generated TikZ text is not compared byte-for-byte because qrab has a different frontend and renderer.
