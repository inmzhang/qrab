...
`set wires to quantum|classical|hidden` changes wire rendering. A transition to `hidden` or `quantum` may add `as "value"` to draw a known-value exit or entry marker. `start` and `end` defer or stop selected wires and may carry an `as "label"`. `bundle` draws a bundle slash, while `label` centers text across a wire span on the diagram background, so the wire breaks around it as it does in qpic. A gate with `shape: none` takes no fill, which is also what qpic does, so its wire runs behind the text. `permute` lists selected wires in their new visual order; later operations and output labels follow that order. `space` reserves invisible room; a `width` wider than the layout's `column_gap` widens that column and shifts everything after it, while a narrower one changes nothing because the abstract gap is a floor and `touch` aligns later operations with the preceding slice. Anything else that outgrows the column gap widens its column the same way, including a gate stretched by an explicit `width` or by a label too long for the default size. An omitted `start` list selects every inactive wire; omitted lists for `end`, `barrier`, `label`/`equals`, `space`, `touch`, `labels`, `brace`, `note`, and `cut` select every active wire. A targetless statement is rejected when that default selection is empty.
...
A vertical circuit reads top to bottom, as qpic's does: the input labels sit above the first column and the wires run left to right in declaration order.

Shapes grow to fit their labels rather than clipping them. Along the time axis that widens the column, so nothing collides. Across the wires there is nowhere to grow into, and a circle or ellipse whose label is wider than `layout.wire_gap` will reach into its neighbours; give the gate a `size` or widen `wire_gap` when that happens. qpic clips instead, which keeps the layout but loses the label.
...
The style grammar is shared so named styles can be reused, but each visual consumes only relevant fields. Gates, measurements, and value markers use the full box/text set; wire declarations use `stroke`, `dash`, and `opacity`; `space` uses only `width` and `height`; `barrier` and `cut` are always dashed and use `stroke` and `opacity`; `touch` uses `stroke`, `dash`, and `opacity`; `swap` and `permute` also use an applicable `width`; labels and endpoints portably use `stroke`, `fill`, `opacity`, and `link`. Other fields are accepted and ignored. In particular, `width` or `link` on a wire declaration, `fill` or `dash: solid` on a barrier or cut, and `shape` on a label or touch do not affect output.

`bundle` styling is backend-specific: LaTeX applies `stroke`, `dash`, and `opacity` to the slash, while Quill 0.8 exposes no style parameters for `nwire`, so Typst uses Quill's default slash and label appearance.
